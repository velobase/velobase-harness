...
function getLemonSqueezyVariantId(order: ProviderOrder): string {
  const metadata = order.productSnapshot?.metadata;
  const meta = metadata && typeof metadata === "object" ? (metadata as JsonRecord) : {};
...
function toLemonCustomData(payment: ProviderPayment, order: ProviderOrder, isSubscription: boolean): Record<string, string> {
  const metadata =
    payment.extra &&
...
async function lemonFetch(path: string, init?: RequestInit) {
  ...
  if (!res.ok) {
    throw new Error(`LemonSqueezy API request failed (${res.status}): ${text || "(empty body)"}`);
  }
  return json;
}
...
async function createCheckout(params: {
  ...
  const variantId = getLemonSqueezyVariantId(params.order);
  const custom = toLemonCustomData(params.payment, params.order, params.isSubscription);
  const successUrl = typeof params.payment.extra?.SuccessURL === "string" ? params.payment.extra.SuccessURL : undefined;
...
function verifyLemonSqueezySignature(rawBody: string, signatureHeader: string | null): boolean {
...
function parseWebhook(rawBody: string, req: Request) {
  ...
  return { payload, meta, data, attributes, customData, eventName };
}

function metadataFromCustomData(customData: JsonRecord) {
  return {
    paymentId: typeof customData.paymentId === "string" ? customData.paymentId : undefined,
    orderId: typeof customData.orderId === "string" ? customData.orderId : undefined,
  };
}
...
function normalizeSubscriptionStatus(attributes: JsonRecord) {
  ...
  if (status === "active" || status === "on_trial" || status === "cancelled") return "SUCCEEDED" as const;
...
export const lemonsqueezyProvider: PaymentProvider = {
  ...
  async confirmPayment() {
    // LemonSqueezy checkout completion is authoritative via signed webhooks.
    return { isPaid: false };
  },
  ...
  async handlePaymentWebhook(req: Request) {
    ...
    if (eventName === "subscription_created") {
      return new BaseWebhookResult({
        status: "SUCCEEDED",
        gatewayTransactionId: typeof attributes.order_id === "number" ? String(attributes.order_id) : undefined,
  ...
  async handleSubscriptionWebhook(req: Request) {
    ...
    if (eventName === "subscription_payment_success" || eventName === "subscription_payment_recovered") {
      ...
      return new BaseWebhookResult({
        ...
        subscriptionPeriod: billingReason === "renewal" || billingReason === "updated" ? 2 : 1,
        amount: toAmountCents(attributes.total),
        currency: typeof attributes.currency === "string" ? attributes.currency.toLowerCase() : undefined,
        rawData: payload,
        isSubscription: true,
    ...
    if (
      ...
      const status = eventName === "subscription_expired" ? "EXPIRED" : normalizeSubscriptionStatus(attributes);
